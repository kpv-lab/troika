...
function getTextRenderInfo(args, callback) {
  ...
  layoutInWorker(args).then(result => {
    ...
    glyphIds.forEach((glyphId, i) => {
      ...
      const posX = glyphPositions[i * 2]
      const posY = glyphPositions[i * 2 + 1]
...
const fontProcessorWorkerModule = /*#__PURE__*/defineWorkerModule({
  ...
  init(config, fontParser, createFontProcessor, bidiFactory) {
    const {defaultFontURL} = config
    return createFontProcessor(fontParser, bidiFactory(), { defaultFontURL })
  }
})

// Fan-out to multiple worker threads:
...
// }
const generateSDFInWorker = defineWorkerModule({
  name: 'SDFGenerator',
  dependencies: [
    CONFIG,
    createGlyphSegmentsIndex,
...
const layoutInWorker = /*#__PURE__*/defineWorkerModule({
  name: 'TextLayout',
  dependencies: [
  ...
  init(fontProcessor, Thenable) {
    return function(args) {
      ...
      fontProcessor.layout(args, thenable.resolve)
      return thenable
    }
  },
  getTransferables(result) {
